...
public class StudentInfoController {
    ...
    public R update(StudentInfoDO studentInfo) {
        ...
        if (studentInfoService.update(studentInfo) > 0) return R.ok();
    ...
    public R remove(Integer id) {
        if (studentInfoService.remove(id) > 0) return R.ok();
    ...
    public R remove(@RequestParam("ids[]") Integer[] ids) {
        if (studentInfoService.batchRemove(ids) > 0) return R.ok();
        else return R.error("删除失败");
    }
    ...
    @PostMapping("/save")
    @ResponseBody
    public R add(StudentInfoDO studentInfo) {
        if ( studentInfoService.save(studentInfo) > 0) return R.ok();
